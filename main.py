    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            try:
                ...
                return JSONResponse(content={"message": resp.text}, status_code=resp.status_code)
        except httpx.ConnectError:
            raise HTTPException(status_code=503, detail="Spring Boot unavailable on port 8080")


# ── AUTH ──────────────────────────────────────────────
# Frontend hits FastAPI first (the gateway), which forwards to Spring Boot,
# which owns the real user DB and issues the token.
...
# ── EXPENSES ──────────────────────────────────────────
# NOTE: paths are "/expenses/*" (plural) to match the frontend (api.js)
# and Spring Boot's @RequestMapping("/expenses") — previously this was
# "/expense/*" (singular), which the frontend never actually calls.
...
async def total_expense(request: Request):
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            r     = await client.get(f"{SPRING_BOOT_URL}/{endpoint}", headers={"Authorization": auth_header})
            data  = r.json()
            total = sum(e.get("amount", 0) for e in data) if isinstance(data, list) else 0
...
# ── PROFILE / ADMIN ───────────────────────────────────
# NOTE: previously these admin routes lived at "/admin/users" and
# "/admin/users/{user_id}", but the frontend (api.js) calls "/profile/all"
# and "/profile/{userId}" directly — those routes didn't exist before.
